import { Armchair, Bus, Coffee, MapPin, ShieldCheck, Snowflake, Wifi, Zap } from "lucide-react";
import { Link } from "react-router-dom";

export const Logo = () => (
  <Link to="/" className="flex items-center gap-2.5 group" aria-label="CityLink home">
    <div className="w-10 h-10 rounded-xl bg-primary flex items-center justify-center shadow-[var(--shadow-elegant)] group-hover:scale-105 transition-transform">
      <Bus className="w-5 h-5 text-primary-foreground" strokeWidth={2.5} />
    </div>
    <div className="flex flex-col leading-none">
      <span className="font-bold text-lg" style={{ fontFamily: "Sora" }}>
        CityLink
      </span>
      <span className="text-[10px] uppercase text-muted-foreground">Modern Luxury Coaches</span>
    </div>
  </Link>
);
